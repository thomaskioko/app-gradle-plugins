...
- Read string and plural keys from the accessor files Moko Resources 0.27.0 generates. Moko 0.27.0 no longer declares each key inside `MR.strings` and `MR.plurals`; it writes every key as an extension property in batch files such as `CommonMainStrings0.commonMain.kt`, leaving the `MR` object empty. `generateMokoStrings` only read `MR.kt`, so it found no keys, wrote empty `StringResourceKey` and `PluralsResourceKey` classes, and every reference to a key failed to compile. The task now reads every file Moko generates for `commonMain` and sorts the keys, so the output does not depend on the order of the batch files. This needs Moko Resources 0.27.0 or later; a project on an earlier version gets empty classes.
- Open the `java.base` packages Robolectric reads through reflection on every test task, following the JVM flags Robolectric documents for JDK 17 and later. Robolectric 4.17 sets the raw file descriptor by reflection when an Android 16 or later SDK boots, and JDK 17 and later refuse that access unless the package is opened, so every module running Robolectric on those SDKs failed before its first test. A consumer no longer needs a `subprojects` block in the root build script to pass the flags itself.
...
See [annotations.md](https://thomaskioko.github.io/app-gradle-plugins/navigation/annotations/), [examples.md](https://thomaskioko.github.io/app-gradle-plugins/navigation/examples/) sections 4, 7, and 8, and [architecture/consumer-contract.md](https://thomaskioko.github.io/app-gradle-plugins/internals/consumer-contract/) for the full surface.
