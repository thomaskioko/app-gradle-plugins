...
- Check how the plugins themselves are written, using the Gradle best practices plugin and the Android lint checks for Gradle code. Both found real faults, listed above. The remaining findings are all one thing, using the root project from a module, which stops Gradle from configuring projects independently. Those are recorded so a new one still fails the build.
